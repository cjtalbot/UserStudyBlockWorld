...
public class InitScript : MonoBehaviour {
	...
	static Texture2D mytexture4;
	static Texture2D mytexture5;
	...
	void Start () {
		...
		mytexture4 = new Texture2D(Screen.width, Screen.height); // purple
		y = 0;
		while (y < mytexture4.height) {
			int x=0;
			while (x < mytexture4.width) {
				mytexture4.SetPixel(x, y, new Color(75f/255f,0f,130f/255f));
				++x;
			}
			++y;
		}
		...
		mytexture5 = new Texture2D(Screen.width, Screen.height); // green
		y = 0;
		while (y < mytexture5.height) {
			int x=0;
			while (x < mytexture5.width) {
				mytexture5.SetPixel(x, y, Color.green);
				++x;
			}
			++y;
		}
	...
	void OnGUI() {
		...
		if (intermission) {
			...
			if (indexNumber == 1 || indexNumber == 0) { // baseline
			...
			} else if (indexNumber == 3) { // nlp
				GUI.DrawTexture(new Rect(0,0,Screen.width, Screen.height), mytexture4, ScaleMode.ScaleToFit, false, 0);
				GUI.Label (new Rect((Screen.width/2) - 130, (Screen.height/2) + 60, Screen.width, 50), "This screen is purple", newstyle);

			} else if (indexNumber == 4) { // rules
				GUI.DrawTexture(new Rect(0,0,Screen.width, Screen.height), mytexture5, ScaleMode.ScaleToFit, false, 0);
				GUI.Label (new Rect((Screen.width/2) - 130, (Screen.height/2) + 60, Screen.width, 50), "This screen is green", newstyle);
		            ...
		            {               
		                if(GUI.Button(new Rect(0, (index*25), dropDownRect.height, 25), ""))
		                {
							...
							if (index == 0 || index == 1) {
								...
								newstyle.normal.background = mytexture5;
								newstyle.normal.textColor = Color.black;
							} else if (index == 5) {
								newstyle.normal.background = mytexture2;
								newstyle.normal.textColor = Color.white;
							} else if (index == 3) {
								newstyle.normal.background = mytexture4;
								newstyle.normal.textColor = Color.white;
							} else {
								newstyle.normal.background = mytexture;
								newstyle.normal.textColor = Color.white;
	...
	void RunPlay() {
		...
		case 2: // random
			...
			if (runshort) {
				...
				GlobalObjs.Horatio.transform.position = new Vector3(-26.4f, 0f, 42.8f);
		...
		case 4:
			...
			if (runshort) {
				// place chars
				GlobalObjs.Hamlet.transform.position = new Vector3(-6.8f, 0f, 42.3f);
				GlobalObjs.Hamlet.transform.rotation = new Quaternion(0f, .6f, 0f, -.8f);
				GlobalObjs.Horatio.transform.position = new Vector3(-3.7f, 0f, 36.8f);
				GlobalObjs.Horatio.transform.rotation = new Quaternion(0f, .7f, 0f, -.8f);
				GlobalObjs.GraveDigger.transform.position = new Vector3(14.2f, 0f, 34.3f);
				GlobalObjs.GraveDigger.transform.rotation = new Quaternion(0f, -.8f, 0f, -.7f);
				GlobalObjs.GraveDiggerTwo.transform.position = new Vector3(49.1f, 0f, 50.2f);
				GlobalObjs.GraveDiggerTwo.transform.rotation = new Quaternion(0f, .6f, 0f, -.8f);
		...
		case 5:
			...
			if (runshort) {
				// place chars
				GlobalObjs.Hamlet.transform.position = new Vector3(0f, 0f, 41.8f);
				GlobalObjs.Hamlet.transform.rotation = new Quaternion(0f, 0.7f, 0f, -0.8f);
				GlobalObjs.Horatio.transform.position = new Vector3(4f, 0f, 39.3f);
				GlobalObjs.Horatio.transform.rotation = new Quaternion(0f, 0.7f, 0f, -0.7f);
				GlobalObjs.GraveDigger.transform.position = new Vector3(11.8f, 0f, 40.3f);
				GlobalObjs.GraveDigger.transform.rotation = new Quaternion(0f, -0.7f, 0f, -0.7f);
				GlobalObjs.GraveDiggerTwo.transform.position = new Vector3(49.1f, 0f, 50f);
				GlobalObjs.GraveDiggerTwo.transform.rotation = new Quaternion(0f, -0.7f, 0f, 0.7f);
				
				GlobalObjs.GraveDiggerTwoFunc.doPickup(GlobalObjs.Lantern);
				GlobalObjs.Shovel.transform.position = new Vector3(12.3f, 0.5f, 36.7f);
				GlobalObjs.Skull1.transform.position = new Vector3(11.9f, 0.5f, 38.8f);
				GlobalObjs.Skull2.transform.position = new Vector3(16.3f, 0.5f, 36f);
				//GlobalObjs.Shovel.transform.position.y = .5f;
